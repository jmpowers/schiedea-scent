...
set.seed(1)
mdshybn1 <- metaMDS(decostand(fvolhybn, method="hellinger"),k=1,try=200, autotransform=F, trace=0)
...
pheatmap(as.matrix(t(fvolhybn[,colSums(pavolhybn)>mincount][srt,]))^(1/3), 
         ...
         annotation_col = data.frame(Species=as.integer(svhybn$SpeciesR)[srt], row.names=rownames(fvolhybn)[srt]), 
...
svhybn$num_compounds <- rowSums(pavolhybn)
svhybn$shannon <- diversity(fvolhybn, index="shannon")
...
obj.mean$ftotal <- NA; obj.mean$ftotal[obj.mean$score=="species"] <- colSums(fvolhybn)
