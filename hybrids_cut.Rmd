...
ph <- pheatmap(heat.mat, 
         ...
         annotation_colors = list(Species=hybcolr), annotation_names_col = F,
...
tests.vols %>%  mutate(lfc_species = log2(pmax(`S. kaalae`,vol_floor)/pmax(`S. hookeri`,vol_floor)), 
  ...
  mutate(Class = Class %>% str_remove("s$") %>% 
           recode("Irregular terpene"="Other terpenoid", "Monoterpene"="Monoterpenoid") %>% factor()) %>% 
  ...
  scale_color_manual(values=c("#BC0060","#027B8C","#5d9400","#E56E00")) +
...
(hybplotn.spec <- hybnplot + 
  ...
  geom_point(data=obj[obj$score=="sites",], aes(fill=SpeciesR), shape=21, size=3, color="white") +
    geom_text(data=obj[obj$score=="species" & obj$occur>nrow(volhybn)*minprop,], aes(label=label), color="black", size=3.8) ) 
```
...
(hybplotn.mean.spec <- 
    ...
    geom_text(data=obj.mean[obj.mean$score=="species" & obj.mean$occur>nrow(volhybn)*minprop,], aes(label=label), color="black",  alpha=1, size=3.8) 
    ) 
```
