...
```{r diversity, fig.height=5, fig.width=7}
#Number of compounds
...
#Total emissions and diversity plots
...
tukeys <- sapply(setNames(paste(names(diversity_vars), ' ~ SpeciesR'), names(diversity_vars)), 
       function(form) lm(form, data = svhybn) %>% multcomp::glht(linfct = multcomp::mcp(SpeciesR="Tukey")) %>% multcomp::cld() %>% `$`(mcletters) %>% `$`(Letters)) %>% 
...
(diversity_plot <-   ggplot(svhybn.long, aes(x=SpeciesR, y=value, fill=SpeciesR))  + 
    facet_wrap("name", scales="free_y", strip.position = "left", nrow=1,
               labeller=as_labeller(diversity_vars))+
...
#TODO hybrid cross directions are intermixed
...
pheatmap(as.matrix(t(ngfvolhyb[svhyb$DN=="Night",colSums(pavolhyb[svhyb$DN=="Night",])>mincount][srt,]))^(1/3), 
...
tests.direction <- tests.glht %>% select(variable, test.direction) %>% unnest(test.direction) %>% 
  mutate(adj.p.value = p.adjust(adj.p.value, method="fdr")) %>% select(variable, direction.pvalue = adj.p.value)
tests.parmean <- tests.glht %>% select(variable, test.parmean) %>% unnest(test.parmean) %>% 
  mutate(adj.p.value = p.adjust(adj.p.value, method="fdr")) %>% select(variable, parmean.pvalue = adj.p.value)
...
tests.emm.diversity <- tests.emm %>% filter(variable %in% names(diversity_vars)) 
tests.all <- tests.all %>% filter(!variable %in% names(diversity_vars)) 
tests.emm <- tests.emm %>% filter(!variable %in% names(diversity_vars)) 
...
ngfvolhybn.top %>% mutate(variable=factor(variable, levels=boxplot.order)) %>% 
  ...
  geom_text(data=tests.all, aes(label=signif.num(parmean.pvalue), x=2.5, y=pmax(par.mean,hyb.mean)*1.5)) + 
  geom_point(data=tests.emm, aes(x=Species, y=pmax(estimate,0), color=Species), shape=4, size=2)+
  geom_point(data=tests.all, aes(x=2.5, y=par.mean, shape="parmean"), size=2)+
  geom_point(data=tests.all, aes(x=2.5, y=hyb.mean, shape="hybmean"), size=2)+
...
ngfvolhybn.top %>% mutate(variable=factor(variable, levels=boxplot.order)) %>% 
  filter(variable %in% names(diversity_vars)) %>% 
  ggplot() +  geom_boxplot(aes(x=Species, y=value, color=Species), width=0.5, outlier.size=1) + 
  geom_text(data=tests.diversity, aes(label=signif.num(parmean.pvalue), x=2.5, y=pmax(par.mean,hyb.mean)*1.5)) + 
  geom_point(data=tests.emm.diversity, aes(x=Species, y=pmax(estimate,0), color=Species), shape=4, size=2)+
