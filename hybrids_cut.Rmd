...
```{r withinplant}
bd.plantsi <-  betadisper(vegdist(decostand(volhybn, method="hellinger"), method="bray"), 
                           group = svhybn$plantsi)
meandist.plantsi <- bd.plantsi$group.distances %>% enframe(name="plantsi", value="centroid_dist") %>%
  left_join(count(svhybn, plantsi)) %>% filter(n>1)#plants with resamples
summary(meandist.plantsi$centroid_dist)
```
...
tibble(group=bd.SpeciesR$group, distance = bd.SpeciesR$distances) %>% 
  bind_rows(bd.plantsi$distances %>% enframe(name="plantsi", value="distance") %>%
  left_join(count(svhybn, plantsi)) %>% filter(distance!=0) %>% mutate(group="Within plant")) %>% 
  ggplot(aes(x=group, y=distance, color=group)) + geom_boxplot() + scale_color_manual(values=hybcolr, guide="none") +
  theme_classic() + labs(x="", y="Distance to centroid")
```
