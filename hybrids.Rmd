...
knitr::opts_chunk$set(comment="", cache=T, warning = F, message = F, 
                      fig.path = "hybrids_plots/", dev="svglite", dev.args=list(fix_text_size=FALSE))
...
#the line above was making volhyb and svhyb not match, until fixed with "new consolidated code"
...
ggsave("output/hybrids/hybcapnight_lump_plant.png", hybplotn.spec, height=8, width=8, type="cairo")
ggsave("output/hybrids/hybcapnight_lump_plant.pdf", hybplotn.spec, height=8, width=8, device=cairo_pdf)
```
...
mdspar
library(Cairo)
CairoPNG(file = "output/hybrids/nmdspar_dupes3.png", bg = "grey70", width=4024,height=4024)
...
anova.cca(cappar2, by="term", permutations=999)
anova.cca(cappar2, by="margin")
axes <- c("CAP1","CAP2")

write.csv(cappar2$CCA$v, "output/hybrids/cappar2_abso_yesoct_nohex.csv")

cappar2_switched <- capscale(decostand(volpar, method="hellinger") ~ StopSunset*SpeciesR, distance="bray", metaMDSdist = F, data=svpar)
anova.cca(cappar2_switched, by="term", permutations=999)
...
ggsave(capplot, file="output/hybrids/capplot3_yesoct_yeshex.pdf", height=150, width=150, units="mm", device=cairo_pdf)
ggsave(capplot, file="output/hybrids/capplot3.eps", height=150, width=150, units="mm", dpi=600, device=cairo_ps)
ggsave(capplot, file="output/hybrids/capplot3.png", height=150, width=150, units="mm", dpi=600)
...
adonis(decostand(sqrt(volpar), method="tot")~specpar*svpar$StopSunset, permutations=999)
adonis(decostand(sqrt(volhyb), method="tot")~specp*svhyb$StopSunset, permutations=999)
...
ggsave(capplotHOOK_nox, file="output/hybrids/capplot3HOOKyesoct_nohex_nox_nodup_abso.pdf", height=150, width=190, units="mm", dpi=600, device=cairo_pdf)
...
png(file = "output/hybrids/barplot3HOOK.png", bg = "white", width=600,height=600)
...
ggsave(capplotKAAL_nox, file="output/hybrids/capplot3KAALyesoct_nohex_nox_nodup_abso.pdf", height=150, width=190, units="mm", dpi=600, device=cairo_pdf)
...
png(file = "output/hybrids/barplot3KAAL.png", bg = "white", width=600,height=600)
...
ggsave(capplotBOTH, file="output/hybrids/capplot3BOTHyesoct_nohex_nox_nodup_abso.pdf", height=300, width=190, units="mm", device=cairo_pdf)
```
...
library(RColorBrewer)
range01 <- function(x){(x-min(x))/(max(x)-min(x))}
points(ophyb, what="sites", col=colorRampPalette(brewer.pal(11,"Spectral"))(100)[round(100*range01(as.numeric(svhyb$StartSunset)))], pch=as.integer(as.factor(hybcolg[as.integer(svhyb$Population)])))
...
cmpd <- c("(Z)-hex-3-en-1-ol", "benzaldehyde", "oct-1-en-3-ol", "unknown benzenoid", "2-phenylacetaldehyde", "α-phellandrene","linalool oxide (pyranoid) ketone","linalool oxide (pyranoid)","linalool oxide (furanoid)")
cmpd <-"oct-1-en-3-ol"
...
ggsave("output/hybrids/oct_vs_hex_relative.pdf", width=6, height=7, device=cairo_pdf)
...
ggsave("output/hybrids/oct_vs_hex_absolute.pdf", width=6, height=7, device=cairo_pdf)
...
svhyb$RawAbundance <- rowSums(volhyb)
...
(ccapplot <-   ggplot(svhybn, aes(x=SpeciesR, y=ccapred, fill=SpeciesR))  +  
    ...
    scale_x_discrete("Species", labels=(c(expression(italic("S. kaalae")), "K x H", "H x K", expression(italic("S. hookeri"))))))
ggsave("output/hybrids/ccaplot3.png", ccapplot, height=6, width =3.5) 
...
```{r lda_dn, eval=FALSE}
range01 <- function(x){(x-min(x))/(max(x)-min(x))}
timeday <- range01(as.numeric(svpar$StartSunset))
library(viridisLite)
